...
**Status:** complete. The direction pick is settled — **2a, “Warm Ledger”**: a cream ledger
sheet in warm ink, one clay accent, square corners, a serif that reads as a printed
statement rather than a dashboard. §2, §3, §4 and §6.4 are transcribed below with measured
values; nothing in this file is pending.
...
### 2.5 The symbol — decided

**“Totals rule.”** Two unequal blocks standing on one heavy rule that spans exactly their
combined width. Nothing else.

Built on a 32×32 grid, from three rectangles and no curves:

| Part | Geometry (32×32 grid) | Fill |
|---|---|---|
| Left block (the larger share) | x 4, y 6, w 12, h 16 | `--accent` `#a8551f` |
| Right block (the smaller share) | x 18, y 12, w 10, h 10 | `--seg-2` `#6b7a4a` |
| Totals rule | x 4, y 25, w 24, h 3 | `--text` `#23281f` |

Why this and not the ruled page or the cable: it is the product's one claim drawn literally.
Two unequal parts, one rule underneath them, and the rule is neither longer nor shorter than
the parts it carries — *the shares add up to the bill exactly*. It is also the same shape as
the signature component (the ratio rail, §8) and the same shape as the totals rule under the
breakdown table, so the mark is the interface's own vocabulary rather than an ornament
borrowed from outside it. At 16px it survives: three solid rectangles, largest feature 12px,
no stroke to thin out, no detail to lose. In one ink the two blocks separate by the 2px gap
and the height step, not by colour — which is what the printed statement and the photocopy
need (§10.4).

Geometry rules: block gap 2px, rule weight 3px, gap between blocks and rule 3px. The height
step between the two blocks is fixed at 6px and does **not** track the real split — the mark
is not a chart and must not be redrawn per cycle.

**Rejected, with the reason, so they don't come back:** the ruled page + check mark reads as
“document approved”, which is a claim about process rather than arithmetic, and its check
mark broke below 20px. The cable cross-section is about electricity, not about fairness —
and the product's whole argument is that it is a money tool, not a meter tool. The
instrument panel's pulse dot implied live telemetry the app does not have.

**Lockup.** Symbol at cap-height × 1.6 (so 25px symbol against the 30px `display` step),
wordmark “Fairmeter” in Fraunces 600 with −0.02em tracking, gap = half the symbol width.
...
Six neutrals plus four hues. These are the only raw values in the codebase; everything else
references a semantic name.

| Primitive | Hex | What it is |
|---|---|---|
...
| `--clay-500` | `#b6602f` | the accent hue at full brightness |
...
Dark-theme primitives: `--ink-950 #191b16` · `--ink-850 #23261e` · `--ink-975 #131510` ·
`--sand-050 #f0ece1` · `--sand-200 #a8a291` · `--ink-700 #3a3d33` · `--ink-500 #5c604f` ·
`--clay-300 #d98a4e` · `--olive-300 #a3b573` · `--slate-300 #7fb0bd` · `--red-400 #e8735c` ·
`--red-950 #3a221d`.

### 3.7 Semantic map — light

Contrast measured, not assumed. Every ratio below is computed against the surface the token
is actually used on.

| Token | Hex | On | Ratio | Floor |
|---|---|---|--:|---|
| `--surface` | `#f2ece1` | — | — | — |
| `--surface-raised` | `#fdfbf6` | — | — | — |
| `--surface-sunk` | `#ebe3d2` | — | — | — |
| `--text` | `#23281f` | raised | **14.5:1** | 4.5 |
| `--text` | `#23281f` | surface | **12.8:1** | 4.5 |
| `--text` | `#23281f` | sunk | **11.7:1** | 4.5 |
| `--text-muted` | `#635e50` | raised | **6.3:1** | 4.5 |
| `--text-muted` | `#635e50` | surface | **5.5:1** | 4.5 |
| `--text-muted` | `#635e50` | sunk | **5.1:1** | 4.5 |
| `--border` | `#d6cdba` | raised | 1.4:1 | n/a — non-text |
| `--border-strong` | `#c3b48f` | raised | 1.8:1 | n/a — non-text |
| `--accent` | `#a8551f` | raised | **5.1:1** | 4.5 |
| `--accent` | `#a8551f` | surface | **4.5:1** | 4.5 |
| `--accent-ink` | `#fdfbf6` | accent | **5.1:1** | 4.5 |
| `--accent-hover` | `#8e461a` | raised | 6.9:1 | 4.5 |
| `--alert` | `#b23a2b` | raised | **5.8:1** | 4.5 |
| `--alert` | `#b23a2b` | alert-soft | **4.9:1** | 4.5 |
| `--alert-soft` | `#f8e6e1` | — | — | — |
| `--ok` | `#5b6a3c` | raised | **5.7:1** | 4.5 |
| `--focus` | `#a8551f` | surface | **4.5:1** | 3.0 |
| `--seg-1` | `#a8551f` | white label on it | **5.3:1** | 4.5 |
| `--seg-2` | `#6b7a4a` | white label on it | **4.7:1** | 4.5 |
| `--seg-3` | `#3f6470` | white label on it | **6.4:1** | 4.5 |

Note the split inside the clay hue: `--clay-500 #b6602f` is 4.3:1 on raised and therefore
**not permitted for body text** — it exists only for decorative fills and hover washes.
`--accent` is `--clay-600 #a8551f`, which clears 4.5:1 both as ink on cream and as ground
under cream. One accent, usable in both directions, is worth more than two that each work
only one way.

### 3.8 Semantic map — dark

Same roles, remapped primitives. Print never uses this set (§10.4).
...
| `--surface-sunk` | `#131510` | — | — |
| `--text` | `#f0ece1` | raised | **13.0:1** |
| `--text` | `#f0ece1` | surface | **14.7:1** |
| `--text-muted` | `#a8a291` | raised | **6.0:1** |
| `--text-muted` | `#a8a291` | surface | **6.8:1** |
| `--border` | `#3a3d33` | raised | 1.5:1 (non-text) |
| `--border-strong` | `#5c604f` | raised | 2.6:1 (non-text) |
| `--accent` | `#d98a4e` | raised | **5.6:1** |
| `--accent-ink` | `#1a1610` | accent | **6.5:1** |
| `--alert` | `#e8735c` | raised | **5.2:1** |
| `--alert-soft` | `#3a221d` | — | — |
| `--ok` | `#a3b573` | raised | **6.9:1** |
| `--focus` | `#d98a4e` | surface | **6.4:1** |
| `--seg-1 / 2 / 3` | `#d98a4e` · `#a3b573` · `#7fb0bd` | dark ink label on each | ≥ 6.0:1 |

Household hues keep their identity across themes — clay, olive, slate in that order — so a
household that is clay on screen is clay in every cycle and, in one ink, is labelled “Ground
floor” with no colour dependency at all.
...
### 4.4 The three faces — decided

| Role | Face | Weights loaded | Fallback stack |
|---|---|---|---|
...
Why this pairing: Fraunces is a ledger serif — high contrast, slightly old-style, the voice
of a printed statement rather than a dashboard — and it is the reason the printed sheet (§10.4)
looks like a document somebody signed. Instrument Sans is neutral and tall-x at 13–15px,
which is the whole job of the body face here. DM Mono has a single-storey `1` and open `0`
with no slash, so a column of readings does not look like a serial number, and its figures
are tabular by default.

**Only five font files load** — Fraunces 600 (variable, subset to latin), Instrument Sans
400/500/600, DM Mono 400/500. No italics: nothing in this product is emphasised by slant.
All three are `display: swap`, and the fallback stacks above are metric-checked so a swap
does not reflow a table.
...
Single column, `max-width: 1040px`, centred. No grid system — there are no columns to
align. Inside a card, fields flow in `repeat(auto-fit, minmax(200px, 1fr))`, which is what
makes the form collapse cleanly on a phone without breakpoint work.

### 6.3 Breakpoints

Two. `600px` (phone → tablet) and `1024px` (→ desktop). Below 600px the layout is one
...
`--shadow-overlay` reserved for anything that floats. **There is no z-index scale** — the
app has no modals, dropdowns or toasts, so nothing stacks. If that changes, add the scale
then, not now.

**Radius — decided (2a).** `--radius-sm: 2px` (inputs, buttons, small controls) ·
`--radius-md: 2px` (cards, tiles) · `--radius-pill: unused` — the direction is a ruled
sheet, and a ruled sheet has corners. 2px rather than 0 only so that a 1px border does not
look chipped at the join. The ratio rail and the table are square, full stop.

**Shadows — decided.** `--shadow-raised: 0 1px 2px rgba(35, 40, 31, 0.06)` — barely there;
the card is separated by its border and its paper tone, not by a lift.
`--shadow-overlay: 0 8px 24px rgba(35, 40, 31, 0.14)`, defined and currently unused.
...
## Still to do (implementation, not decisions)

The brand is settled; what remains is landing it in the codebase.

1. Rebuild `app/src/styles.css` on the token structure in §7, using the primitives in §3.6
   and the maps in §3.7–3.8.
2. Draw `symbol.svg`, `lockup.svg` and `favicon.svg` from the §2.5 geometry table, plus the
   inline React component so the mark inherits `--accent`, `--seg-2` and `--text`.
3. Swap the three faces in, drop the old font links, and confirm no table reflows on swap.
4. Rename the app's `<h1>` and `index.html` `<title>` to Fairmeter.
5. Re-measure the two ratios sitting exactly on the 4.5:1 line — `--accent` on `--surface`
   and `--seg-2` under a white label — in the browser after the faces land, since
   anti-aliasing at 11px is where a nominal pass stops being a real one.
